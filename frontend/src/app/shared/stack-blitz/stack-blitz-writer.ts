...
const COPYRIGHT = `Copyright ${new Date().getFullYear()} cdot65. All Rights Reserved.
...
export class StackBlitzWriter {
    ...
    createStackBlitzForExample(
        ...
        return this._ngZone.runOutsideAngular(async () => {
            const files = await this._buildInMemoryFileDictionary(
                data,
                exampleId,
                isTest
            );
        ...
        for (const relativeFilePath of TEMPLATE_FILES) {
            tasks.push(
                this._loadFile(TEMPLATE_PATH + relativeFilePath)
                    ...
                    .then((content) =>
                        this._replaceExamplePlaceholders(
                            data,
                            relativeFilePath,
                            content,
                            isTest
        ...
        for (const relativeFilePath of data.exampleFiles) {
            ...
            tasks.push(
                this._loadFile(exampleBaseContentPath + relativeFilePath)
                    ...
                    .then((content) =>
                        this._appendCopyright(relativeFilePath, content)
                    )
    ...
    private _replaceExamplePlaceholders(
        ...
        if (fileName === "src/index.html") {
            ...
            const importFileName = data.indexFilename.slice(
                0,
                dotIndex === -1 ? undefined : dotIndex
            );
            fileContent = fileContent.replace(
                /material-docs-example/g,
                importFileName
            );
        }
        return fileContent;
