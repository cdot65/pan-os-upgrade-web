...
const TEST_URLS = TEMPLATE_FILES.map(
    (filePath) => `/assets/stack-blitz/${filePath}`
).concat([
...
describe("StackBlitzWriter", () => {
    ...
    beforeEach(() => {
        ...
        EXAMPLE_COMPONENTS[testExampleId] = {
            importPath: "cdk/my-comp",
        } as LiveExample;
    });
    ...
    it("should append correct copyright", () => {
        ...
        expect(stackBlitzWriter._appendCopyright("test.ts", "NoContent"))
            .toBe(`NoContent

/**  Copyright ${year} cdot65. All Rights Reserved.
    Use of this source code is governed by an MIT-style license that
    can be found in the LICENSE file at https://angular.io/license */`);

        expect(stackBlitzWriter._appendCopyright("test.html", "NoContent"))
            .toBe(`NoContent
...
/**  Copyright ${new Date().getFullYear()} cdot65. All Rights Reserved.
    Use of this source code is governed by an MIT-style license that
    can be found in the LICENSE file at https://angular.io/license */`,
            /* eslint-enable @typescript-eslint/naming-convention */
        });
        ...
        expect(openProjectSpy).toHaveBeenCalledTimes(1);
        expect(openProjectSpy).toHaveBeenCalledWith(
            jasmine.objectContaining({ files: expectedFiles }),
